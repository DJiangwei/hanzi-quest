...
### A2 — Cross-week review loop `[x]` (both slices shipped)
Two cheap slices, in order:
1. **Stale-char distractors** `[x]` (shipped 2026-09-05, PR #175): when compiling a new week, draw some MCQ distractors from *previously cleared weeks'* characters. Passive re-exposure, zero new UI. **Cheaper than this doc assumed** — found while building slice 2: four of the five MCQ types — `audio_pick`, `image_pick`, `translate_pick`, `sentence_cloze` — store only the TARGET's identity in `scene_config` and sample their distractors at RUNTIME from a `pool` prop already threaded to `SceneRunner`/`BossScene`, so widening that pool needs no `recompile-all-weeks.ts` run for them. The one exception is `image_word`, which freezes `distractorWordIds` at compile time and so does need a recompile; `lianliankan` is not an MCQ at all — it freezes the four `characterIds` that make up its board. The real work is re-deriving PR #158's cross-week stimulus-ambiguity guard (`validStimulusWords`) across the widened pool — exactly what 温故's `buildWordOwners` had to do for its own cross-week pool (see the CLAUDE.md landmine on it) — or the 唱歌-style collision returns one week over.
...
### V1 — Mastery model + visible growth (航海日志 / Captain's Logbook) `[x]` (shipped 2026-09-04, PR #167)
...
**Shipped as:** a coarse three-state `masteryForChar` (`src/lib/mastery/mastery.ts`) rather than 0–3 stars — 57 of production's 96 characters carry only 1–2 scored answers, so four levels would have been precision the data cannot support. `MASTERY_MIN_EVIDENCE=3` / `PROFICIENT_ACCURACY=0.8` yield 31 熟练 / 8 学习中 / 57 unbadged today; unrated characters appear as entries with no badge rather than being hidden or marked down. Pure, no stored column, no gating, as the guardrails required. 温故's `reviewScore` was rewritten onto the same function in the same PR — a live dilution fix, since its `count(*)` denominator counted flashcard rows that can never be `correct = false`. See CLAUDE.md's 航海日志 paragraph + its three landmines.
**Follow-up — the degenerate `got_it` signal `[~]` (collection fixed 2026-09-05, PR #168; awaiting data).** All 164 flashcard self-ratings in production are `got_it`; not one `not_sure`, not one `dont_know`, ever. That is why the mastery denominator has to drop them, and it is almost certainly a `FlashcardScene` UX problem (认识 being the fastest way past the card) rather than a true report. **PR #168 addressed the collection side**: the emit path was verified sound first (buttons, schema and write all correct), so the constant was bias — 认识 sat first AND glowed emerald while 不认识 was rose, in a game where those colours mean correct and wrong everywhere else. The three buttons are now styled identically, and `answer_events.revealed` (migration 0043) records whether she uncovered the pinyin or meaning before rating, which self-report bias cannot reach. **Still open, and only data can close it:** if `revealed` differentiates over the next few weeks while the self-rating stays 164/164 `got_it`, then a three-way self-assessment simply does not work on a six-year-old, and the answer is to replace the question with a non-judgemental one ("练这个字还要再来一次吗?") rather than to keep restyling the buttons.

### V2 — Smart distractor selection `[~]` (slice 1 shipped 2026-09-06, PR #185 — tone neighbours in audio_pick; shape/component similarity is slice 2)
...
**Unblocked by V1:** distractor difficulty can now weight on `masteryForChar` — reach for that function rather than inventing a third scorer, so the Logbook, 温故 and V2 never disagree about how well a character is known (CLAUDE.md landmine: *There is ONE mastery function and two readers*).
...
Tones are the known weak point (MeloTTS failure; UK-based kid). **Shipped as a STANDALONE surface** (`/play/[childId]/tones`, Backpack entry) rather than a compiled scene type — no `scene_templates` row, no compile-week slot, no recompile — because the premise is unverified and the cost of being wrong should stay low. Her own corpus turns out to support it richly: **29 minimal-pair groups covering 68 of 176 characters**, including 妈/马/吗 and 十/是/石, and the bank grows by itself as each new week adds colliding syllables. The device is given the HANZI, so its tone is correct by construction. Choices are hanzi, never pinyin — pinyin is hidden by default (locked decision) and 'which tone number' is a metalinguistic label a six-year-old does not need. **Premise verified 2026-09-05:** David played it on the device and confirmed the tones are audibly distinct. The game survives, and the deliberate no-rewards v1 did its job — nothing had to be unwound.

**PR #178 then fixed what playing it exposed:** the builder padded every question up to four options from anywhere in her corpus, and a probe showed that of the 20 syllables she knows in more than one tone, NOT ONE is known in four. A filler option is eliminable by reading, so a padded four-way question was *easier* than the honest two-way one and rewarded scanning for the familiar shape. Choices now come only from the answer's own syllable; two-option questions are kept.

**Remaining slice — decided by David 2026-09-06: wire rewards AND fold it into weekly practice.** Design constraints inherited from the rest of the product: the reward must pay for COMPLETION, never for score (the 温故 rule — a review that punishes wrong answers is a test); nothing anywhere may GATE on tones having been played (boss unlock still counts practice only); and if it becomes a compiled practice slot it needs a `scene_templates` row, a `compile-week.ts` slot and a `recompile-all-weeks.ts` run post-merge, which the standalone version deliberately avoided. A candidate shape that keeps the cheap path: leave the standalone surface as-is, add a `'tones'` card-grant source paying once per UTC day like 温故, and reach practice integration only if the daily surface is actually opened.
...
- `[x]` **Multi-buy furniture** — shipped 2026-07-18 (PR #145): cap 3 copies per furniture item (wallpaper/floor stay 1), per-copy placement identity, tray ×N badges.
