...
**Open follow-up — the degenerate `got_it` signal.** All 164 flashcard self-ratings in production are `got_it`; not one `not_sure`, not one `dont_know`, ever. That is why the mastery denominator has to drop them, and it is almost certainly a `FlashcardScene` UX problem (认识 being the fastest way past the card) rather than a true report. **Until someone looks at its button layout — button order, or requiring a beat before advancing — a third of A1's telemetry is a constant.** Worth a small UX PR; it costs signal every session it stays unexamined.
