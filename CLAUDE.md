...
**Shipped:** PR #1 → #32. The product is end-to-end playable in production: weekly authoring, AI scene generation (DeepSeek V4 Pro), shared `pirate-class-level-1` curriculum pack with 10 weeks published, island map, 9 scene types + boss, coins (with daily-login / streak-milestone / perfect-week bonuses + a `BonusToast` HUD layer), **5 collection packs** in the Collector's Atlas (`/play/[childId]/collection` → museum of packs, per-pack `/collection/[packSlug]` pages): zodiac (12), flags (30), sea creatures (20), dinosaurs (15), solar system (10). **Shop hub** (`/play/[childId]/shop` → tabbed UI, Avatar tab live with ~20 procedural-SVG cosmetics, other tabs "即将上线"). Layered SVG avatar in play HUD, animations + audio + treasure-map cards, PWA manifest.
...
**Next up (per `docs/superpowers/specs/2026-05-18-pr21-shop-expansion-design.md` roadmap):** Remaining shop expansion — consumable powerups (hint / skip / streak-freeze), pet companion, island decorations. ALWAYS confirm with David before starting a new PR.
