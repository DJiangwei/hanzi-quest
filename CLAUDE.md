...
**Next up:** **PR #41 — Pollinations image backfill** (Gemini AI Studio free-tier blocks image gen, so we pivoted to Pollinations.ai which is free and key-less). Write `scripts/backfill-word-images.ts` calling `https://image.pollinations.ai/prompt/<urlencoded>` for each of the 238 existing words, upload to Vercel Blob (token already provisioned), populate new `words.image_url` column (migration 0016). Swap `ImageWordScene` stimulus to `<img>` with description-card fallback. Also wire into authoring (`generate-week-v1.ts`) so future weeks auto-generate. Beyond that next PRs are open-ended. ALWAYS confirm with David before starting a new PR.
