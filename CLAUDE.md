...
- **PR #38 (just shipped, 2026-05-26)** — Kid-first surface refresh. Five subsystems in one PR: (1) root `/` auto-redirects signed-in single-child accounts to `/play/[childId]`; (2) bottom `KidNavBar` (🏝️ Map / 🎒 背包 / 📅 日历 / 🛒 商店 + ⚙️ gear) mounted in play layout, mid-scene tap shows quit-confirm dialog via `MidSceneProvider` context; (3) parent dashboard gated behind 4-digit PIN (bcrypt, HttpOnly 15-min cookie, 5-attempt → 5-min lockout, `?reset=1` recovery) — new `parent_settings` table (migration 0014), new `/parent/unlock` page + `POST /api/parent-unlock`; (4) View Transitions API morph island circle ↔ hub hero via matching `viewTransitionName: island-${weekId}`; new `loading.tsx` Suspense skeleton; (5) 7-day check-in strip on home + month-grid `/calendar` page driven by `getActivityForRange` aggregation over `coin_transactions`; (6) Atlas renamed to **背包 / Backpack** with 🎒 header + "最近获得 / Recently Obtained" strip showing last 3 acquisitions across collection / avatar / pet / decor with a 24h NEW sticker (server-render snapshot via `nowMs` prop). New dep: `bcryptjs`. 20 new tests across 12 new test files (485 → 505). The Flux image swap originally queued for PR #38 is now deferred (Gemini MCP `media-pipeline` configured user-scope, `gemini-2.5-flash-image` default; needs a backfill script + `words.image_url` column).

**Next up:** **two open paths**. (a) **Image generation backfill** — write `scripts/backfill-word-images.ts` against the now-configured Gemini MCP / API to populate `words.image_url` for the 238 existing words (~$9 at gemini-2.5-flash-image), add `words.image_url` column, swap `ImageWordScene` stimulus to `<img>` with description-card fallback. (b) **PR B from the original brainstorm** — multi-map / chapter system: name the existing 10-week pack as "Map 1" (e.g. 大航海时代发现顺序) and add a placeholder "Map 2" gateway for future curriculum. Beyond that next PRs are open-ended (more curriculum content, parent dashboard polish, etc.). ALWAYS confirm with David before starting a new PR.
